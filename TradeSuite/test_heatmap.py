...
def create_sample_result_data():
    ...
    # Create sample action details
    action_details = {
        'VWAP_Mean_Reversion': {
            'signals': [1 if i % 20 == 0 else 0 for i in range(len(time_index))]
        },
        'Order_Block_Detection': {
            'signals': [1 if i % 15 == 0 else 0 for i in range(len(time_index))]
        },
        'FVG_Detection': {
            'signals': [1 if i % 25 == 0 else 0 for i in range(len(time_index))]
        }
    }
...
def test_heatmap():
    ...
    # Create sample data
    sample_data = create_sample_result_data()
    ...
    print("Results viewer window opened with sample data.")
    print("Navigate to the 'Heatmap' tab to see the building blocks heatmap.")
    print("The heatmap should show:")
    print("- VWAP_Mean_Reversion signals")
    print("- Order_Block_Detection signals") 
    print("- FVG_Detection signals")
