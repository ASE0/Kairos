"""
Test strategy integration without GUI
"""
...
class TestStrategyIntegration(unittest.TestCase):
    ...
    def test_pattern_strategy(self):
        """Test pattern strategy evaluation"""
        signals, action_details = self.pattern_strategy.evaluate(self.data)
    ...
    def test_risk_strategy(self):
        """Test risk strategy evaluation"""
        signals, risk_details = self.risk_strategy.evaluate(self.data)
        ...
        # Check risk details
        self.assertIsInstance(risk_details, pd.DataFrame)
        self.assertEqual(len(risk_details), len(self.data))
        
        # Check risk columns
        self.assertIn('atr', risk_details.columns)
        self.assertIn('stop_loss_atr', risk_details.columns)
        self.assertIn('take_profit_atr', risk_details.columns)
        self.assertIn('risk_per_trade', risk_details.columns)
        self.assertIn('trailing_stop', risk_details.columns)
        self.assertIn('trailing_stop_atr', risk_details.columns)
        
        # Check risk values
        self.assertEqual(risk_details['stop_loss_atr'].iloc[0], 2.0)
        self.assertEqual(risk_details['take_profit_atr'].iloc[0], 3.0)
        self.assertEqual(risk_details['risk_per_trade'].iloc[0], 0.02)
        self.assertEqual(risk_details['trailing_stop'].iloc[0], False)
        self.assertEqual(risk_details['trailing_stop_atr'].iloc[0], 1.5)
    
    def test_combined_strategy(self):
        """Test combined strategy evaluation"""
